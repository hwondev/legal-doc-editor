...
- 사이트 호스팅 설정 (Vercel) — `vercel.json`, `vite.site.config.ts`(데모 페이지를 앱으로 빌드해 `site/`), 판례 검색 함수 `api/cases.js`(인증값은 환경변수 `LAW_GO_KR_OC`, 검색어 1~100자, 같은 검색어 1시간 CDN 캐시). 사이트에서는 실제 판례 검색, 로컬 개발에서는 예시 결과. 문서가 브라우저 안에서만 처리된다는 안내 문구

### Fixed

- `searchCases`가 목록이 아닌 값을 돌려주면(연결 설정이 잘못돼 HTML이 오는 경우 등) 에디터가 통째로 사라지던 문제 — 판례 검색 패널에 안내만 띄움
...
- 템플릿 선택을 툴바의 **템플릿…** 선택에서 패널 목록으로 옮김 (고르면 문서를 바꾸고, 작성 중인 내용이 있으면 확인하는 동작은 그대로)
...
- 데모가 페이지 바깥의 템플릿 선택 목록 대신 에디터 안 **템플릿…** 선택을 씀
...
- `groupClauses`가 조항뿐 아니라 템플릿도 묶음 (`category`·`title`이 있는 목록이면 됨, 기존 호출 그대로)

### Changed

- `templates` 순서를 분류 순서(원고·신청인 → 피고 → 기타)로 맞춤. 첫 템플릿은 그대로 소장 (대여금)

## [0.19.0] - 2026-09-16

### Added

- 피고용 준비서면 템플릿 `preparatory-brief-defendant` — "위 사건에 관하여 피고는…", 원고 주장에 대한 반박, 피고의 추가 주장, 원고가 낸 증거에 대한 의견, 결론("원고의 청구는 이유 없습니다"), `을` 호증과 본문 참조, 첨부서류 통수. 사건 표시의 원고·피고 순서는 그대로

### Changed

- 기존 준비서면 템플릿 이름을 **준비서면 (원고)**로 바꿈 (id `preparatory-brief`는 그대로)
...
- `autoFees`가 `{{준비서면 부본 통수}}`(상대방 수, 민사소송법 제273조)도 채움
...
- `autoFees`가 `{{답변서 부본 통수}}`(상대방 수, 민사소송법 제256조제3항·민사소송규칙 제48조제1항)도 채움
- 불러오기에서 "청구취지에 대한 답변"·"청구원인에 대한 답변"을 소제목으로 인식 (워드로 저장했다 다시 열어도 소제목 유지)
...
- 소장(대여금)·소장(채무부존재확인) 템플릿 첨부서류가 `위 입증방법 각 1통`으로 고정되어 있던 것을 통수 변수로 바꿈 (피고 1명이면 각 2통)
...
- 호증 시작 번호 — 호증 문단 속성 `data-evidence-start`(`<ol start>`처럼 그 문단을 정한 번호로, 다음 호증은 이어서, 당사자별), 툴바 **호증 시작** 버튼, `setEvidenceStart` 명령. 화면·내보내기·본문 참조가 같은 번호를 씀.
  불러오기는 입증방법 아래 그 당사자의 첫 호증이 `갑 제5호증`처럼 1이 아니어도 시작 번호로 받고, 본문 참조는 목록에 있는 번호만 참조로 바꿈. 시작 번호 문단 맨 앞에서 Enter로 위에 호증을 넣으면 시작 번호가 새 첫 문단으로 옮겨감
...
- 호증 범위·나열 참조 자동 갱신 — `갑 제1호증 내지 제3호증`, `갑 제1호증 ~ 제3호증`, `갑 제1호증, 제2호증`, `갑 제1, 2호증`, `갑 제1 내지 3호증`의 번호마다 자기 증거를 가리킴. 참조 모양 `data-form`(전체 `갑 제1호증` / 뒤쪽 `제3호증` / 번호만 `2`).
  불러오기는 묶음 안 모든 번호의 증거가 있을 때만 변환하고, 입력은 참조 뒤에 ` 내지 제3호증`을 이어 쳐도 뒤쪽 번호가 참조로 바뀜. 번호 나열 뒤 가지번호(`갑 제1, 2호증의 1`)는 글자로 둠
