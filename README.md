...
- **입증방법 호증 번호**: `갑 제1호증`, `갑 제2호증`… 증거를 중간에 넣거나 빼도 호증 번호가 알아서 다시 매겨지고, 본문의 `(갑 제1호증)` 참조도 따라 바뀌어요
...
| `p[data-evidence="갑"\|"을"\|"병"]` | 호증 문단 (입증방법) | 갑 제1호증 → 갑 제2호증, 당사자별로 문서 전체에서 이어 셈. 번호 문단과 함께 쓰면 `1. 갑 제1호증`. `data-evidence-start="5"`면 그 문단부터 5로 시작 |
| `span[data-evidence-ref]` | 본문 호증 참조 | 가리키는 호증 문단(`data-evidence-id`)의 현재 번호. `data-form="short"`는 `제3호증`, `"number"`는 `3`만. 증거가 없어지면 빨간 밑줄 |
| `table` | 표 | 표 |
| `ol > li` | 항 | ①, ② |
| `ol ol > li` | 호 | 1., 2. |
| `span[data-var]` 또는 `{{이름}}` | 변수 | 입력값 |

툴바 **호증** 버튼은 누를 때마다 갑 → 을 → 해제로 바뀌고, 호증 문단 끝에서 Enter를 누르면 다음 호증이 이어져요. 준비서면처럼 앞 서면에 낸 증거 다음 번호부터 적을 때는 첫 호증 문단에서 **호증 시작**을 눌러 번호(예: 5)를 정하면 `갑 제5호증`, `갑 제6호증`…으로 이어져요. 첫 호증을 지우거나 위에 호증을 새로 넣어도 시작 번호는 지금 첫 호증으로 옮겨가요. 파일을 열 때는 입증방법·증명방법·증거방법 소제목 아래의 `갑 제N호증`이 차례대로일 때만 호증 문단으로 바꿔요(첫 번호가 5면 5부터 시작하는 것으로 봐요). `갑 제1호증의 1` 같은 가지번호나 `내지`로 묶은 줄은 글자로 둬요.

본문(청구원인 등)에 `갑 제2호증`을 입력하면 그 증거를 가리키는 **참조**로 바뀌어요(바로 Backspace를 누르면 글자로 돌아가요). 증거를 넣고 빼거나 잘라내 옮겨도 참조 번호가 따라 바뀌고, 가리키던 증거를 지우면 빨간 밑줄로 알려 줘요. 파일을 열 때도 자동 번호로 바꾼 증거가 있으면 본문의 `갑 제N호증`을 참조로 바꿔요. `갑 제1호증 내지 제3호증`, `갑 제1호증, 제2호증`, `갑 제1, 2호증` 같은 범위·나열도 번호마다 자기 증거를 따라가요(`갑 제1호증`을 친 뒤 ` 내지 제3호증`을 이어 쳐도 돼요). 묶음 안에 문서에 없는 증거가 섞였거나 `갑 제1, 2호증의 1`처럼 번호 나열 뒤에 가지번호가 붙으면 글자로 두니 번호가 바뀌면 직접 고쳐 주세요.
...
`templates`를 넘기면 오른쪽 패널에 **템플릿** 목록이 생겨요. 검색칸에서 이름으로 찾거나 분류 칩(전체·원고·신청인·피고·기타)으로 좁히고, 누르면 문서가 그 템플릿으로 바뀌어요. 작성 중인 내용이 있으면 먼저 물어보고, 바꾼 뒤에도 되돌리기(Ctrl·⌘+Z)로 돌아올 수 있어요. 입력값은 그대로 두어서 같은 이름의 빈칸(`{{원고 이름}}` 등)에 이어서 쓰여요.
...
첨부서류의 `{{입증방법 통수}}`·`{{소장 부본 통수}}`는 소가가 없어도 상대방 수로 채워요(피고 1명이면 `위 입증방법 각 2통`, `소장 부본 1통`).
...
| 첨부서류 서증 사본 통수 | `{{입증방법 통수}}` = 상대방 수 + 1. 상대방 수 기본은 당사자 수 − 1 — `autoFees={{ opponents: 2 }}`. **전자소송(`electronic`)은 채우지 않음** | 「민사소송규칙」 제105조제2항 |
| 소장·답변서·준비서면 부본 통수 | `{{소장 부본 통수}}`·`{{답변서 부본 통수}}`·`{{준비서면 부본 통수}}` = 상대방 수 | 「민사소송규칙」 제48조제1항, 「민사소송법」 제256조제3항(답변서 부본은 원고에게 송달)·제273조(준비서면 부본은 상대방에게 송달), [대한법률구조공단 첨부서류 작성방법](https://support.klac.or.kr/front/contents/lawsuit/011.do) |

> 계산 결과는 **참고용**이에요. 제1심 소장 기준이고 항소·상고·반소는 계산하지 않아요. 실제 납부액은 법원 안내를 확인하세요.

> 전자소송(`autoFees={{ electronic: true }}`)에서는 첨부서류 통수를 채우지 않아요. 전자소송 동의를 한 상대방에게 보낼 부본·사본은 내지 않아도 되고([민사소송 등에서의 전자문서 이용 등에 관한 업무처리지침](https://portal.scourt.go.kr/pgp/main.on?w2xPath=PGP1051M04&c=900&jisCntntsSrno=2025000019074&srchwd=*&originDvsCd=07&rnum=3&pgDvs=1) 제23조제3항), 그 밖의 상대방에게는 법원이 출력해 송달하기 때문이에요(「민사소송 등에서의 전자문서 이용 등에 관한 법률」 제12조제1항). 다만 출력할 분량이 50쪽 이상이거나 상대방이 5명 이상이면 법원이 출력서면 제출을 명할 수 있어요(같은 지침 제23조제1항).
...
| `Numbering` | 번호·호증 문단 확장 (`setNumbering(1~4 \| null)`, `setEvidence('갑' \| '을' \| '병' \| null)`, `setEvidenceStart(n \| null)`, Tab·Shift+Tab 단계 변경, 맨 앞 Backspace 해제) |
| `EvidenceRef` | 본문 호증 참조 확장 (`LegalEditor`에는 이미 들어 있음). `evidenceLabels(editor.getJSON())`로 호증 id별 현재 번호 |
