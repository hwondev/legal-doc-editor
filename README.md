...
| `span[data-evidence-ref]` | 본문 호증 참조 | 가리키는 호증 문단(`data-evidence-id`)의 현재 번호. 증거가 없어지면 빨간 밑줄 |
| `table` | 표 | 표 |
| `ol > li` | 항 | ①, ② |
| `ol ol > li` | 호 | 1., 2. |
...
본문(청구원인 등)에 `갑 제2호증`을 입력하면 그 증거를 가리키는 **참조**로 바뀌어요(바로 Backspace를 누르면 글자로 돌아가요). 증거를 넣고 빼거나 잘라내 옮겨도 참조 번호가 따라 바뀌고, 가리키던 증거를 지우면 빨간 밑줄로 알려 줘요. 파일을 열 때도 자동 번호로 바꾼 증거가 있으면 본문의 `갑 제N호증`을 참조로 바꿔요. `갑 제1호증 내지 제3호증`, `갑 제1호증, 제2호증` 같은 범위는 글자로 두니 번호가 바뀌면 직접 고쳐 주세요.
