...
툴바 **호증** 버튼은 누를 때마다 갑 → 을 → 해제로 바뀌고, 호증 문단 끝에서 Enter를 누르면 다음 호증이 이어져요. 준비서면처럼 앞 서면에 낸 증거 다음 번호부터 적을 때는 첫 호증 문단에서 **호증 시작**을 눌러 번호(예: 5)를 정하면 `갑 제5호증`, `갑 제6호증`…으로 이어져요. 파일을 열 때는 입증방법·증명방법·증거방법 소제목 아래의 `갑 제N호증`이 차례대로일 때만 호증 문단으로 바꿔요(첫 번호가 5면 5부터 시작하는 것으로 봐요). `갑 제1호증의 1` 같은 가지번호나 `내지`로 묶은 줄은 글자로 둬요.
